...
var AppView = Backbone.View.extend({
    initialize: function(options) {
        ...
        this.activeMarkers = [];
        this.coordinatesMatrix = new CoordinateMatrix();
        ...
        for (var i = markers.length - 1; i >= 0; i--) {
            ...
            var value = this.coordinatesMatrix.getValue(lat,lng)
    ...
    getDevices: function(bounds) {
        ...
        for(var lng = swLng; lng <= neLng; lng++){
            for(var lat = swLat; lat <= neLat; lat++){
                devices = $.extend(devices,this.coordinatesMatrix.getValue(lat,lng))
            }
        }
